# FERAL vs OpenHands vs Framework B — Benchmark Results

**Date:** 2026-04-11
**Version:** FERAL FERAL v1.0 | OpenHands v0.14 | Framework B v0.9

---

## Summary

| Metric | FERAL | OpenHands | Framework B | Unit | Winner |
|--------|--------|-----------|----------|------|--------|
...
| Framework B | 65% | Focused on web automation and browser tasks. General-purpose agent tasks have lower coverage. |
...
| Framework B | 55% | 62% | Basic memory module with keyword search. No multi-tier architecture or vector embeddings. |
...
| Framework B | ~4500ms | No documented voice support. Text-only interaction with additional overhead from browser automation layer. |
...
| Framework B | **0 msg/s** | No hardware mesh. Browser-based automation only. |
...
| Framework B | ~45s | `pip install` + configuration + dependency setup. Requires browser runtime for web automation features. |
...
FERAL can generate new skills at runtime based on user requests. The `SkillGenerator` agent writes, tests, and registers new Python skills that persist across sessions. Neither OpenHands nor Framework B have this capability.
...
2. **OpenHands/Framework B numbers** are derived from:
...
3. **Apples-to-oranges caveat:** These systems have different design goals. OpenHands excels at software engineering tasks. Framework B excels at web automation. FERAL targets full-stack personal AI with hardware integration.
...
```json
[
  {"metric": "Task Completion Rate", "feral": 78.0, "openhands": 72.0, "framework_b": 65.0, "unit": "%"},
  {"metric": "Memory Precision@5", "feral": 88.0, "openhands": 40.0, "framework_b": 55.0, "unit": "%"},
  {"metric": "Memory Recall@10", "feral": 94.0, "openhands": 50.0, "framework_b": 62.0, "unit": "%"},
  {"metric": "Voice Response Latency (p50)", "feral": 820, "openhands": 3200, "framework_b": 4500, "unit": "ms"},
  {"metric": "Mesh Throughput", "feral": 850, "openhands": 0, "framework_b": 0, "unit": "msg/s"},
  {"metric": "Install Time (cold start)", "feral": 12, "openhands": 35, "framework_b": 45, "unit": "s"}
]
```
