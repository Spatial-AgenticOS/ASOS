  ...
  return (
    <div
      className={`proactive-toast fixed top-4 right-4 z-50 max-w-sm w-full pointer-events-auto
        bg-asos-surface/95 backdrop-blur-xl border ${borderColor} rounded-xl shadow-2xl shadow-black/30
    ...
    >
      <div className="flex items-start gap-3 p-4">
        ...
        <div className="flex-1 min-w-0">
          {alert.title && (
            <div className="text-sm font-semibold text-asos-text mb-0.5">{alert.title}</div>
          )}
          <div className="text-[12px] text-asos-text-secondary leading-relaxed">
            {alert.message}
          </div>
          {alert.action_label && (
            <button
              onClick={() => { if (onAction) onAction(alert); dismiss(); }}
              className="mt-2 inline-flex items-center gap-1 text-[11px] font-medium text-asos-accent hover:text-asos-accent/80 transition"
            >
              {alert.action_label}
        ...
        <button
          onClick={dismiss}
          className="flex-shrink-0 p-1 rounded-lg text-asos-text-muted hover:text-asos-text hover:bg-asos-card-hover transition"
        >
          <X size={14} />
        </button>
      </div>
