...
| `publish.yml` | tag `v*`; `workflow_dispatch` (input: `dry_run`) | Staged release pipeline: **build** (wheel/sdist + in-tree pre-publish smoke) → **stage** (publish to TestPyPI, install from TestPyPI, runtime smoke against the uploaded artifact; gated by `testpypi-stage` environment) → **publish** (PyPI push + GitHub Release; gated by `pypi` environment). Production publish requires successful stage. Canary smoke is driven by `scripts/release_wheel_smoke.py`. |
| `install-smoke.yml` | `workflow_run` after Release; `workflow_dispatch` | Post-prod `pip install feral-ai` on Linux matrix (Ubuntu × Py 3.11/3.12) by default, verify CLI, assert webui_v2 in site-packages, uvicorn `/` smoke. Optional macOS lanes are manual-only via `workflow_dispatch` input. Runs *after* the staged `publish.yml` has pushed to real PyPI. |
