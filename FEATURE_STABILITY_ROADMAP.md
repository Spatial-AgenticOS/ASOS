...
| `publish.yml` | tag `v*` | Build wheel/sdist for `feral-core`, GitHub Release, **PyPI publish**. |
| `install-smoke.yml` | `workflow_run` after Release; `workflow_dispatch` | `pip install feral-ai`, verify CLI, assert webui_v2 in site-packages, uvicorn `/` smoke. |
