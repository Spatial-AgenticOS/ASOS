...
# FERAL CLI Demo — interactive terminal agent
...
if ! curl -sf http://localhost:9090/ >/dev/null 2>&1; then
  ...
  echo "  Start it first: docker compose up -d  OR  cd feral-core && PYTHONPATH=. python api/server.py"
  exit 1
fi
...
if ! command -v feral &>/dev/null; then
  echo -e "  Installing FERAL CLI..."
  pip install -e . -q
fi

echo -e "\n  ${BOLD}Starting FERAL CLI...${NC}\n"
exec feral "$@"
