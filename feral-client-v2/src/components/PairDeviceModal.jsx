...
function WebPhoneTab({ onIssue }) {
  ...
  // React StrictMode (dev) double-invokes effects; rapid re-mount could
  // also run the auto-generate twice. The ref guarantees exactly one
  // generation per mount.
  const autoGenerated = useRef(false);

  const generate = useCallback(async () => {
    setBusy(true);
    setError(null);
    const promise = apiJson('/api/devices/pair/url?name=web-phone');
    onIssue?.(promise);
    try {
