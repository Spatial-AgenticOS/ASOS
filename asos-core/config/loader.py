"""
THEORA Config Loader — Layered Configuration System
=====================================================
Inspired by claw-code-parity's ConfigLoader: merges settings from
multiple sources in priority order.

Hierarchy (highest priority wins):
  1. Environment variables (THEORA_*)
  2. Local config  (.theora/settings.local.json) — machine-specific, gitignored
  3. Project config (.theora/settings.json) — shared with team
  4. User config    (~/.theora/settings.json) — user-global defaults

Credentials are stored separately in ~/.theora/credentials.json
and NEVER merged into settings (blind vault pattern).

Skills are discovered from:
  - ~/.theora/skills/           (user-installed)
  - .theora/skills/             (project-local)
  - Built-in manifests in asos-core/skills/manifests/
"""
...
logger = logging.getLogger("theora.config")
...
def theora_home() -> Path:
    """Resolve the THEORA user config directory (XDG-compliant on Linux)."""
    env_home = os.environ.get("THEORA_HOME")
    if env_home:
        return Path(env_home)

    xdg_config = os.environ.get("XDG_CONFIG_HOME")
    if xdg_config:
        return Path(xdg_config) / "theora"

    return Path.home() / ".theora"


def theora_data_home() -> Path:
    """Resolve the THEORA data directory (XDG-compliant on Linux)."""
    xdg_data = os.environ.get("XDG_DATA_HOME")
    if xdg_data:
        return Path(xdg_data) / "theora"
    return Path.home() / ".theora"


class ConfigLoader:
    """
    Loads and merges THEORA configuration from multiple sources.
    """

    def __init__(self, project_dir: Optional[str] = None):
        self.user_home = theora_home()
        self.data_home = theora_data_home()
    ...
    def discover(self) -> dict:
        ...
        # Layer 1: User config (~/.theora/settings.json)
        user_path = self.user_home / "settings.json"
        self._load_and_merge(user_path, "user")

        # Layer 2: Project config (.theora/settings.json)
        project_path = self.project_dir / ".theora" / "settings.json"
        self._load_and_merge(project_path, "project")

        # Layer 3: Local config (.theora/settings.local.json) — gitignored
        local_path = self.project_dir / ".theora" / "settings.local.json"
    ...
    def _apply_env_overrides(self):
        """Map THEORA_* environment variables to config keys."""
        env_map = {
            "OPENAI_API_KEY": None,  # handled by credentials
            "THEORA_LLM_PROVIDER": ("llm", "provider"),
            "THEORA_LLM_MODEL": ("llm", "model"),
            "THEORA_LLM_BASE_URL": ("llm", "base_url"),
            "THEORA_VISION_ENABLED": ("vision", "enabled"),
            ...
            "THEORA_TTS_VOICE": ("audio", "tts_voice"),
    ...
    def _load_credentials(self):
        ...
        # Skill-specific keys from THEORA_KEY_* pattern
        for key, value in os.environ.items():
            if key.startswith("THEORA_KEY_"):
                skill_id = key[11:].lower()  # THEORA_KEY_web_search → web_search
    ...
    def discover_skills_directories(self) -> list[Path]:
        ...
        # Project-local
        project_skills = self.project_dir / ".theora" / "skills"
    ...
    def export_as_env(self) -> dict[str, str]:
        ...
        env["THEORA_LLM_PROVIDER"] = llm.get("provider", "openai")
        env["THEORA_LLM_MODEL"] = llm.get("model", "gpt-4o-mini")
        if llm.get("base_url"):
            env["THEORA_LLM_BASE_URL"] = llm["base_url"]

        vision = self._merged.get("vision", {})
        env["THEORA_VISION_ENABLED"] = str(vision.get("enabled", False)).lower()
        env["THEORA_VISION_MAX_FRAME_KB"] = str(vision.get("max_frame_kb", 512))
