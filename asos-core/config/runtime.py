"""
THEORA Runtime Contract helpers.
...
def brain_bind_host() -> str:
    return os.getenv("THEORA_HOST") or os.getenv("THEORA_BIND_HOST") or "0.0.0.0"


def brain_port() -> int:
    return _int_env("THEORA_PORT", "THEORA_BRAIN_PORT", default=9090)


def brain_public_scheme() -> str:
    return os.getenv("THEORA_PUBLIC_SCHEME", "http")


def brain_public_host() -> str:
    return (
        os.getenv("THEORA_PUBLIC_HOST")
        or os.getenv("THEORA_BRAIN_HOST")
        or "localhost"
    )


def brain_public_port() -> int:
    return _int_env("THEORA_PUBLIC_PORT", "THEORA_BRAIN_PORT", "THEORA_PORT", default=9090)


def brain_public_base_url() -> str:
    explicit = os.getenv("THEORA_PUBLIC_BASE_URL", "").strip()
    if explicit:
...
def market_registry_url() -> str:
    return os.getenv("THEORA_MARKETPLACE_URL", "http://localhost:8080/api/v1")


def ollama_base_url() -> str:
    return os.getenv("THEORA_OLLAMA_BASE_URL", "http://localhost:11434")
