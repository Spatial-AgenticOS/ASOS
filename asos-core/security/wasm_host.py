"""
THEORA WASM Host Functions — The ABI bridge between sandbox and THEORA
========================================================================
...
logger = logging.getLogger("theora.wasm_host")
...
class WASMHostFunctions:
    ...
    def theora_log(self, memory: bytearray, msg_ptr: int, msg_len: int):
        """Log a message from the WASM skill."""
        if not self._check_allowed("theora_log"):
            return
        msg = memory[msg_ptr:msg_ptr + msg_len].decode("utf-8", errors="replace")
        self._logs.append(msg)
        logger.info(f"[WASM] {msg[:200]}")

    def theora_get_param(self, memory: bytearray, name_ptr: int, name_len: int) -> tuple[int, int]:
        """Read a tool call parameter. Returns (ptr, len) in WASM memory."""
        if not self._check_allowed("theora_get_param"):
            return (0, 0)
    ...
    def theora_set_result(self, memory: bytearray, data_ptr: int, data_len: int):
        """Set the return value of the WASM skill execution."""
        if not self._check_allowed("theora_set_result"):
            return
        self._result = memory[data_ptr:data_ptr + data_len].decode("utf-8", errors="replace")

    def theora_http_get(self, memory: bytearray, url_ptr: int, url_len: int) -> tuple[int, int]:
        """Make an HTTP GET request (gated by policy network rules)."""
        if not self._check_allowed("theora_http_get"):
            return (0, 0)
