...
const RealAudioPlayer = ({ url, label, action_id, onAction }) => {
  ...
  return (
    <div className="w-full bg-feral-card border border-feral-border rounded-xl p-3 flex items-center gap-3">
      <audio
        ref={audioRef}
        src={url}
        onTimeUpdate={() => {
      ...
      <button
        onClick={togglePlay}
        className="w-10 h-10 rounded-full bg-feral-accent flex items-center justify-center hover:scale-105 transition flex-shrink-0"
      >
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="w-full h-1 bg-feral-border rounded-full mt-1">
          <div className="h-full bg-feral-accent rounded-full transition-all" style={{ width: `${progress}%` }} />
        </div>
      </div>
    </div>
  );
...
const GraphView = ({ nodes = [], links = [], height = 280 }) => {
  ...
  return (
    <div className="w-full bg-feral-card border border-feral-border rounded-xl overflow-hidden">
      <svg viewBox={`0 0 ${w} ${h}`} className="w-full" style={{ maxHeight: h }}>
        ...
        {nodes.map((node, i) => {
          ...
          return (
            <g key={id}>
              <circle cx={p.x} cy={p.y} r={10} fill="#06b6d4" opacity={0.9} />
              <text
                x={p.x}
                y={p.y + 22}
                textAnchor="middle"
...
const ChartView = ({ data, chart_type, label, color, height }) => {
  ...
  return (
    <div className="w-full">
      ...
      <canvas ref={canvasRef} width={400} height={height || 150} className="w-full bg-feral-card border border-feral-border rounded-xl" />
    </div>
  );
};
...
const FormView = ({ fields, action_id, submit_label, onAction }) => {
  ...
  return (
    <form onSubmit={handleSubmit} className="w-full flex flex-col gap-3 bg-feral-card border border-feral-border rounded-xl p-4">
      {fields && fields.map((field, i) => (
        <div key={i} className="flex flex-col gap-1">
          <label className="text-xs opacity-70">{field.label || field.name}</label>
          {field.type === 'select' ? (
            <select
              className="bg-feral-bg border border-feral-border rounded-lg px-3 py-2 text-sm"
            ...
            <textarea
              className="bg-feral-bg border border-feral-border rounded-lg px-3 py-2 text-sm resize-none"
              rows={3}
      ...
      <button type="submit" className="px-4 py-2 bg-feral-accent text-white rounded-lg font-medium hover:bg-feral-accent/90 transition">
        {submit_label || 'Submit'}
      </button>
    </form>
  );
  ...
  return (
    <div className="flex items-center justify-between w-full">
      ...
      <button
        ...
        className={`relative w-11 h-6 rounded-full transition-colors ${on ? 'bg-feral-accent' : 'bg-feral-border'}`}
      >
  ...
  if (mediaType === 'video' || (src && (src.endsWith('.mp4') || src.endsWith('.webm')))) {
    return (
      <div className="w-full">
        ...
        <video src={src} controls className="w-full rounded-xl border border-feral-border" style={{ maxHeight: height || 300 }} />
      </div>
    );
  }
  return (
  ...
  return (
    <div className="w-full flex flex-col gap-1">
      {(sections || []).map((section, i) => (
        <div key={i} className="border border-feral-border rounded-lg overflow-hidden">
          <button
            onClick={() => setOpenIdx(openIdx === i ? -1 : i)}
            className="w-full flex items-center justify-between px-3 py-2 bg-feral-card hover:bg-white/5 transition text-sm font-medium"
          >
...
const WebViewComponent = ({ url, height, sandbox }) => (
  <iframe
    src={url}
    className="w-full border border-feral-border rounded-xl"
...
const TableView = ({ headers, rows, color }) => (
  <div className="w-full overflow-x-auto">
    <table className="w-full text-sm border border-feral-border rounded-xl overflow-hidden">
      {headers && (
        <thead>
          <tr className="bg-feral-card">
            {headers.map((h, i) => (
              <th key={i} className="text-left px-3 py-2 font-medium opacity-80 border-b border-feral-border">{h}</th>
            ))}
          </tr>
        </thead>
      )}
      <tbody>
        {rows && rows.map((row, i) => (
          <tr key={i} className="border-b border-feral-border last:border-0">
...
const CodeBlock = ({ code, language }) => (
  <pre className="w-full bg-feral-bg border border-feral-border rounded-xl p-4 overflow-x-auto text-sm font-mono">
    <code>{code}</code>
  </pre>
);
...
const TerminalBlock = ({ fields }) => {
  ...
  return (
    <div className="bg-feral-bg border border-feral-border rounded-lg overflow-hidden text-[11px] font-mono">
      <div className="flex items-center gap-1.5 px-2.5 py-1 border-b border-feral-border/50 bg-feral-card/60">
        <span className={`w-1.5 h-1.5 rounded-full ${exitCode === '0' || exitCode === '' ? 'bg-emerald-400' : 'bg-rose-400'}`} />
        <span className="text-feral-text-muted text-[10px]">
          {exitCode !== '' ? `exit ${exitCode}` : 'shell'}
        </span>
      </div>
      {lines.length > 0 && (
        <pre className="px-2.5 py-1.5 text-feral-text whitespace-pre-wrap break-words leading-relaxed max-h-32 overflow-y-auto">
          {lines.join('\n')}{truncated ? '\n…' : ''}
        </pre>
      )}
      {!stdout && !stderr && (
        <div className="px-2.5 py-1.5 text-feral-text-muted italic">Done.</div>
      )}
      {stderr && (
        <pre className="px-2.5 py-1.5 text-rose-400/80 whitespace-pre-wrap break-words leading-relaxed border-t border-feral-border/50 max-h-20 overflow-y-auto">
          {stderr.slice(0, 300)}
        </pre>
      )}
    </div>
...
export const SduiRenderer = ({ node, onAction, compact = false }) => {
  ...
  switch (type) {
    ...
    case 'Card':
      return (
        <div className={`sdui-fade-in bg-feral-card border border-feral-border backdrop-blur-md shadow-md flex flex-col ${sp(spacing || 12)} ${compact ? 'p-2.5' : 'p-3'}`}
    ...
    case 'Divider':
      return <div className="w-full h-px bg-feral-border my-2" />;
    case 'Button':
      return (
        <button
          ...
          className={`${compact ? 'px-3 py-1.5 text-xs' : 'px-4 py-2 text-sm'} rounded-lg font-medium transition-all hover:scale-[1.02] active:scale-95 ${
            style === 'primary' ? 'bg-feral-accent text-white hover:bg-feral-accent/90'
            : style === 'danger' ? 'bg-red-600 text-white hover:bg-red-500'
            : 'bg-feral-card border border-feral-border hover:bg-white/10'
          }`}>
          {label}
        </button>
      );
    ...
    case 'MetricCard':
      return (
        <div className={`sdui-metric-pop bg-feral-card border border-feral-border backdrop-blur-md shadow-md ${compact ? 'p-2.5' : 'p-3'} flex flex-col items-center gap-1.5`}
    ...
    case 'ProgressBar':
      return (
        <div className="w-full">
          ...
          <div className="w-full h-2 bg-feral-border rounded-full overflow-hidden">
            <div className="h-full rounded-full transition-all duration-500"
              style={{ width: `${Math.max(0, Math.min(100, (node.value || 0) * 100))}%`, backgroundColor: color || '#06b6d4' }} />
          </div>
        </div>
      );
    case 'MapView':
    ...
    case 'VideoPlayer':
      return (
        <video src={node.url || url} controls className="w-full rounded-xl border border-feral-border"
      ...
      return (
        <div className="w-full animate-pulse">
          {Array.from({ length: node.lines || 3 }).map((_, i) => (
            <div key={i} className="h-4 bg-feral-border rounded mb-2" style={{ width: `${70 + Math.random() * 30}%` }} />
          ))}
        </div>
      );
    case 'Spacer':
