...
export default function CommandPalette({ open, onClose, onCommand, onToggle }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-start justify-center pt-[20vh] palette-backdrop" onClick={onClose}>
      <div
        className="palette-panel w-full max-w-md bg-feral-surface/95 backdrop-blur-2xl border border-feral-border-bright rounded-2xl shadow-2xl shadow-black/40 overflow-hidden"
        onClick={e => e.stopPropagation()}
      >
        <div className="flex items-center gap-3 px-4 py-3 border-b border-feral-border">
          <Search size={16} className="text-feral-text-muted flex-shrink-0" />
          <input
            ref={inputRef}
            type="text"
            value={query}
            ...
            className="flex-1 bg-transparent text-sm text-feral-text placeholder:text-feral-text-muted outline-none"
          />
          <kbd className="hidden sm:inline text-[10px] text-feral-text-muted bg-feral-card px-1.5 py-0.5 rounded border border-feral-border font-mono">
            esc
          </kbd>
        </div>
        ...
        <div className="max-h-72 overflow-y-auto py-1">
          {filtered.length === 0 && (
            <div className="px-4 py-6 text-center text-sm text-feral-text-muted">No results</div>
          )}
          {sections.map(section => {
            const sectionItems = filtered.filter(i => i.section === section);
            return (
              <div key={section}>
                <div className="px-4 pt-2 pb-1 text-[10px] text-feral-text-muted uppercase tracking-wider">{section}</div>
                    ...
                    <button
                      ...
                      className={`w-full flex items-center gap-3 px-4 py-2 text-left text-sm transition-colors ${
                        idx === selected
                          ...
                          : 'text-feral-text-secondary hover:bg-feral-card-hover'
                      }`}
                    >
        ...
        <div className="flex items-center justify-between px-4 py-2 border-t border-feral-border text-[10px] text-feral-text-muted">
