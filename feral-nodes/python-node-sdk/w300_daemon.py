...
class W300Node:
    ...
    async def connect_brain(self):
        """Connect to the FERAL Brain and run event loops."""
        while self.running:
            try:
                async with websockets.connect(self.brain_ws_url) as ws:
                    self.brain_ws = ws
                    logger.info("Connected to FERAL Brain. Registering Glasses...")
    ...
    async def _telemetry_loop(self):
        """Structured FERAL telemetry loop with expanded sensor channels."""
        try:
            while self.running:
                vitals = {
