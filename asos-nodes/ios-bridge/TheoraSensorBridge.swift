/**
 THEORA Sensor Bridge
 =====================
 Connects the existing W300SensorManager (JWBle SDK) to the
 ASOS Brain via ASOSBrainClient. Drop-in integration:

 1. W300SensorManager reads BLE sensor data from THEORA glasses
 2. TheoraSensorBridge receives callbacks and forwards to Brain
...
class TheoraSensorBridge {
    ...
    private let brainClient: ASOSBrainClient
    private var monitoringTimers: [TheoraSensorType: Timer] = [:]
    ...
    init(brainClient: ASOSBrainClient) {
    ...
    /**
     ...
     W300SensorManager API but route through ASOS instead of
     directly to OpenAI.
    ...
    */
    ...
    func readHeartRate() {
        ...
        W300SensorManager.shared.getHeartRate { [weak self] result in
            ...
            case .failure(let error):
                print("[THEORA Bridge] HR error: \(error)")
            }
        }
        #else
    ...
    func readSpO2() {
        ...
        W300SensorManager.shared.getSpO2 { [weak self] result in
            ...
            case .failure(let error):
                print("[THEORA Bridge] SpO2 error: \(error)")
            }
        }
        #else
    ...
    func readTemperature() {
        ...
        W300SensorManager.shared.getTemperature { [weak self] result in
            ...
            case .failure(let error):
                print("[THEORA Bridge] Temp error: \(error)")
            }
        }
        #else
    ...
    func readUV() {
        ...
        W300SensorManager.shared.getUVLevel { [weak self] result in
            ...
            case .failure(let error):
                print("[THEORA Bridge] UV error: \(error)")
            }
        }
        #else
    ...
    func readSteps() {
        ...
        W300SensorManager.shared.getSteps { [weak self] result in
            ...
            case .failure(let error):
                print("[THEORA Bridge] Steps error: \(error)")
            }
        }
        #else
    ...
    private func scheduleReading(_ sensor: TheoraSensorType, interval: TimeInterval, action: @escaping () -> Void) {
