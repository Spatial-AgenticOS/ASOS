"""
Tests for FERAL identity workspace: YAML identity, markdown files, and tool sync.
"""
...
def workspace_home(tmp_path: Path) -> Path:
    """Isolated FERAL home directory."""
    return tmp_path / "feral_identity"
...
class TestIdentityWorkspaceInit:
    ...
    def test_creates_directory_structure(self, workspace_home: Path) -> None:
        ...
        assert "FERAL" in (workspace_home / "IDENTITY.yaml").read_text()
