"""
Focused JSON-RPC and HTTP surface tests for `mcp.server.FeralMCPServer`.
...
class TestFeralMCPServerCore:
    """Construction and protocol entrypoints."""

    def test_init_creates_instance(self) -> None:
        """Server stores optional dependencies and server metadata."""
        srv = FeralMCPServer(device_registry=None, memory=None, perception=None)
        assert srv._devices is None
        assert srv._server_info["name"] == "feral"
    ...
    async def test_jsonrpc_tools_list_returns_tool_definitions(self) -> None:
        ...
        assert "feral_list_devices" in names
            ...
            resp = await srv.handle_jsonrpc(
                {
                    ...
                    "id": 2,
                    "method": "tools/call",
                    "params": {"name": "feral_list_devices", "arguments": {}},
                }
            )
