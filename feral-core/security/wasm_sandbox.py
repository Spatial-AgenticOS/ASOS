"""
FERAL WASM Sandbox — Secure execution of untrusted skill code
================================================================
Uses wasmtime-py to run WASM modules with:
  ...
  - FERAL API exposed through host functions only
...
logger = logging.getLogger("feral.wasm_sandbox")
...
class WASMSandbox:
    ...
    def _register_host_functions(self, linker, store):
        """Register FERAL host functions in the WASM linker."""
        import wasmtime
        ...
        def log_fn(caller, msg_ptr: int, msg_len: int):
            ...
            host.feral_log(mem_bytes, msg_ptr, msg_len)

        def get_param_fn(caller, name_ptr: int, name_len: int) -> int:
            mem = host_memory.data_ptr(caller)
            mem_bytes = bytearray((wasmtime.ffi.c_ubyte * host_memory.data_len(caller)).from_address(mem))
            ptr, length = host.feral_get_param(mem_bytes, name_ptr, name_len)
            return ptr

        def set_result_fn(caller, data_ptr: int, data_len: int):
            mem = host_memory.data_ptr(caller)
            mem_bytes = bytearray((wasmtime.ffi.c_ubyte * host_memory.data_len(caller)).from_address(mem))
            host.feral_set_result(mem_bytes, data_ptr, data_len)

        try:
            # Register host functions under "feral" namespace
            log_type = wasmtime.FuncType([wasmtime.ValType.i32(), wasmtime.ValType.i32()], [])
            linker.define(store, "feral", "log", wasmtime.Func(store, log_type, log_fn))

            get_param_type = wasmtime.FuncType(
                [wasmtime.ValType.i32(), wasmtime.ValType.i32()],
                [wasmtime.ValType.i32()],
            )
            linker.define(store, "feral", "get_param", wasmtime.Func(store, get_param_type, get_param_fn))

            set_result_type = wasmtime.FuncType([wasmtime.ValType.i32(), wasmtime.ValType.i32()], [])
            linker.define(store, "feral", "set_result", wasmtime.Func(store, set_result_type, set_result_fn))

            linker.define(store, "feral", "memory", host_memory)
