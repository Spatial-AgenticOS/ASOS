# FERAL Plugin & Skill SDK

This guide explains how **skills** plug into FERAL: declarative manifests, Python implementations, registration, security, testing, and publishing.
...
**`vault`**: Map of `skill_id → API key` from `FERAL_KEY_<skill_id>` and related config. Use `get_api_key(vault, fallback_env="OPENAI_API_KEY")` for env fallbacks.
...
3. **Marketplace**: Installed packages under `~/.feral/skills/<package>/` with a `manifest.json` are loaded by `SkillRegistry._load_marketplace_skills()` (see `skills/package.py`, `skills/marketplace.py`).

After adding a skill, restart the API or reload the registry so the identity workspace and tool lists stay in sync.

## Security

- **Vault**: Store secrets as `FERAL_KEY_<skill_id>` or via the BlindVault path used by the executor. The LLM never receives raw keys.
...
- Uses `OPENWEATHER_API_KEY` or `FERAL_KEY_weather_current` when set; otherwise falls back to **wttr.in** (no key).
...
4. **Install**: Target `~/.feral/skills/<id>/` so the registry picks up `manifest.json` on the next load.
...
- `_feral_meta` on each tool carries `skill_id`, `endpoint_id`, `ui_hint`, and brand for clients.
...
| 401 from HTTP skill | `FERAL_KEY_<skill_id>` or manifest `auth` headers. |
...
| Variable | Role |
|----------|------|
| `FERAL_KEY_<skill_id>` | API key stored for declarative or Python skills |
| `OPENAI_API_KEY`, `GROQ_API_KEY`, … | LLM providers (separate from skills) |
| `FERAL_HOME` | Config directory (`~/.feral` default); marketplace skills live under `skills/` there |
