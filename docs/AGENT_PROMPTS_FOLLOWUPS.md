...
| 2026-04-24 | W10 (F-W10-006) | `feral-nodes/android-bridge/gradlew:7,50` | The hand-rolled `gradlew` script sets `DEFAULT_JVM_OPTS='"-Xmx64m" "-Xms64m"'` and then runs `exec "$JAVACMD" $DEFAULT_JVM_OPTS …` without `eval`, so Java sees the literal string `"-Xmx64m"` and reports `Could not find or load main class "-Xmx64m"`. The W10 Android workflow currently bypasses this by using `gradle/actions/setup-gradle@v3` and invoking `gradle` directly. The bridge owner should either replace this script with the standard `gradle wrapper`-generated one (which uses `eval` correctly) OR change line 50 to `eval exec "$JAVACMD" $DEFAULT_JVM_OPTS …`. Out of W10 owned paths (`feral-nodes/android-bridge/**` is read-only). | W16 (android-bridge) |
| 2026-04-24 | W10 (F-W10-007) | `feral-registry/tests/test_publish_flow.py` (conftest fixture) | `app_client` uses `importlib.reload(models_mod)` in the per-test fixture, which re-executes `class Publisher(Base):` against the same SQLAlchemy `Base.metadata`, raising `Table 'publishers' is already defined for this MetaData instance` on the second invocation. 38/39 registry tests pass; this one is currently `--deselect`-ed in `.github/workflows/registry.yml`. Fix: either move the reload-fixture to `scope="module"` so it runs once, or drop the reload pattern and use `Base.metadata.clear()` + a fresh declarative base per test. Out of W10 owned paths (`feral-registry/**` is read-only). | W16 (registry-tests) |
