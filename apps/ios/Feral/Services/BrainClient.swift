...
// MARK: - Brain Client (adapted from FeralBrainClient)
...
final class BrainClient: NSObject, ObservableObject {
    ...
    private let sendQueue = DispatchQueue(label: "ai.feral.brain.send", qos: .userInitiated)
    ...
    init(host: String = "localhost", port: Int = 9090, apiKey: String = "") {
        ...
        self.apiKey = apiKey
        #if os(iOS)
        self.nodeId = "feral-iphone-\(UIDevice.current.name.lowercased().replacingOccurrences(of: " ", with: "-"))"
        #else
        self.nodeId = "feral-apple-device"
        #endif
        super.init()
    }
